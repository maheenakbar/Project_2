...
def grab_headlines():
    ...
    #gets the div tag of where the most-read articles are
    div = soup.find('div', class_ = 'panel-pane pane-mostread')
    li = div.find_all('li')
    for item in li:
        #this contains the actual title name
...
def get_umsi_data():
    ...
    #while loop so that it gets the data from each page
    while page < 14:
        soup = BeautifulSoup(html.text, "html.parser")

        div = soup.find('div', class_ = 'view-content')
        #gets all the names in the staff list
        names = div.find_all('div', property = 'dc:title')
        #gets all the titles of the staff list
        titles = div.find_all('div', class_ = 'field field-name-field-person-titles field-type-text field-label-hidden')
        titles_list = []

        #creates a list of the all the titles of the UMSI staff
        for item in titles:
            titles_list.append(item.text)

        index = 0
        for item in names:
            #creats the staff person's name as the key and their title as the value
            umsi_titles[item.text] = titles_list[index]
            index += 1
        
        #goes to the next page
        url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All' + next_pages + str(page)
        html = html = requests.get(url, headers={'User-Agent': 'SI_CLASS'})
        page += 1
